...
<template>
  <nav class="page-tabs" aria-label="已打开页面">
    ...
    <a-dropdown v-if="tabs.items.length" :trigger="['click']">
      <a-button class="tabs-more" type="text" aria-label="标签页操作"><MoreOutlined /></a-button>
      <template #overlay>
        <a-menu
          :items="[
            { key: 'refresh', label: '刷新当前' },
            { key: 'close', label: '关闭当前', disabled: tabs.items.find((item) => item.path === currentPath)?.fixed },
            { key: 'others', label: '关闭其他' },
            { key: 'right', label: '关闭右侧' },
          ]"
          @click="handleAction(String($event.key), currentPath)"
        />
      </template>
    </a-dropdown>
  </nav>
...
.tabs-more { margin: auto 0 auto 4px; color: var(--shell-muted); }
</style>
